...
export class EstadisticasService {
  ...
  async obtenerEstadisticas(): Promise<object> {
    ...
    const proyectosFinalizados = await this.proyectosRepository.count({
    ...
    const tareasFinalizadas = await this.tareasRepository.count({
      where: { estado: EstadosTareasEnum.FINALIZADA },
    });

    const proyectosBaja = await this.proyectosRepository.count({
      where: { estado: EstadosProyectosEnum.BAJA },
    });
    const tareasBaja = await this.tareasRepository.count({
      where: { estado: EstadosTareasEnum.BAJA },
    });

    const totalClientes = await this.clientesRepository.count();
    const clientesActivos = await this.clientesRepository.count({
      where: { estado: EstadosClientesEnum.ACTIVO },
    });
    const clientesBaja = await this.clientesRepository.count({
